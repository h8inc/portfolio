...
const defaultData = [
  12, 24, 36, 52, 68, 85, 103, 122, 142, 165, 189, 214, 241,
  269, 300, 334, 370, 408, 449, 492, 530, 552, 565, 579, 583
];
...
export const AppsAnalyticsCard = ({
  ...
  useEffect(() => {
    const updateWidth = () => {
      if (containerRef.current) {
        ...
        const usableWidth = Math.max(180, containerWidth - 16);
        const computedBarWidth = Math.max(4, usableWidth / barCount);
        const computedChartWidth = computedBarWidth * barCount;
        setBarWidth(computedBarWidth);
        setChartWidth(computedChartWidth);
      }
    };
    updateWidth();
  ...
  const displayValue = hoveredIndex !== null ? data[hoveredIndex] : value;

  // @return
  return <div ref={containerRef} className={`relative flex flex-col w-full max-w-md rounded-[30px] p-[26px] pb-[30px] overflow-hidden cursor-pointer content-card-shell ${className}`} style={{
    backgroundColor
  }}>
      <div className="relative w-full h-full flex flex-col gap-2">
        <div className="flex flex-col gap-1 text-[#130F25] uppercase" style={{ fontFamily: 'Aeonik Extended' }}>
        ...
        >
          {data.map((dataPoint, index) => {
            ...
            const y2 = valueToY(dataPoint);
            const isLast = index === data.length - 1;
            const isHovered = index === hoveredIndex;
            const strokeColor =
              isHovered || isLast ? accentColor : 'rgba(27, 27, 74, 0.1)';
            return (
              <g key={index}>
        ...
        <div className="flex justify-between mt-2 uppercase text-[#130F25] w-full" style={{ fontFamily: 'Aeonik' }}>
          {months.map((month, index) => (
            <div
              key={index}
              className="flex-1 text-center text-[10px] opacity-70 flex items-center justify-center"
              style={{
        ...
        <div className="flex justify-between mt-1 font-medium uppercase opacity-30 text-[#130F25] w-full" style={{ fontFamily: 'Aeonik' }}>
