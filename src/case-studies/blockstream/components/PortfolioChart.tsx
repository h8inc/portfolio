...
const generateDailyData = (days: number) => {
  ...
  let value = 69514; // Start at current Bitcoin price
  for (let i = 0; i < days; i++) {
    // Add realistic daily volatility
    const change = (Math.random() - 0.5) * 2500;
    value = Math.max(66000, Math.min(72000, value + change));
    data.push({
      value
    });
  }
...
export const PortfolioChart = ({
  ...
  useEffect(() => {
    ...
    // Gradient for revealed area
    const gradient = document.createElementNS('http://www.w3.org/2000/svg', 'linearGradient');
    gradient.setAttribute('id', `area-gradient-${variant}`);
    ...
    revealedClip.setAttribute('id', `revealed-clip-${variant}`);
    ...
    unrevealedClip.setAttribute('id', `unrevealed-clip-${variant}`);
    ...
    const areaPathData = linePathData + ` L ${points[points.length - 1].x} ${containerHeight - margin.bottom}` + ` L ${points[0].x} ${containerHeight - margin.bottom} Z`;

    // Create REVEALED area (with gradient, clipped to left of pointer)
    const revealedArea = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    revealedArea.setAttribute('d', areaPathData);
    revealedArea.setAttribute('fill', `url(#area-gradient-${variant})`);
    revealedArea.setAttribute('clip-path', `url(#revealed-clip-${variant})`);
    ...
    revealedLine.setAttribute('stroke', '#00d4ff');
    ...
    revealedLine.setAttribute('clip-path', `url(#revealed-clip-${variant})`);
    ...
    unrevealedLine.setAttribute('stroke', '#00d4ff');
    ...
    unrevealedLine.setAttribute('clip-path', `url(#unrevealed-clip-${variant})`);
    unrevealedLine.setAttribute('opacity', '0.3');
    ...
    verticalLine.setAttribute('stroke', 'rgba(0, 212, 255, 0.4)');
    ...
    hoverDot.setAttribute('fill', '#00d4ff');
    ...
    const handleInteraction = (clientX: number, clientY: number) => {
      ...
      if (closestIndex >= 0 && closestIndex < data.length) {
        ...
        // Calculate tooltip position relative to the container, not fixed to viewport
        const containerRect = containerRef.current!.getBoundingClientRect();
        const tooltipY = margin.top - (isMobile ? 12 : 33);

        // Smart horizontal positioning: calculate pill position with edge detection
        const tooltipWidth = isMobile ? 110 : 130;
        const tooltipHalfWidth = tooltipWidth / 2;
        const pointRelativeX = point.x;
  ...
  if (isMobile) {
    return <div className={`w-full bg-transparent flex flex-col overflow-x-hidden ${className || ''}`}>
  ...
  // Web variant
  return <div className={`w-full bg-[#0f1419] overflow-x-hidden ${className || ''}`}>
      <div className="w-full max-w-7xl mx-auto pb-6">
        {/* Chart Section - Takes 25-30% of viewport height */}
        <div className="w-full" style={{
        height: '27vh',
        minHeight: '250px'
      }}>
          <div ref={containerRef} className="relative w-full h-full bg-[#0f1419] overflow-hidden">
            <svg ref={svgRef} className="w-full h-full"></svg>
